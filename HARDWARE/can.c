...
u8 CAN_Mode_Init(u8 tsjw, u8 tbs2, u8 tbs1, u16 brp, u8 mode)
{
    ...
    /*** 过滤器初始化 ***/
		// 过滤器组工作在初始化模式（FINIT=1，允许修改过滤器寄存器）
		CAN1->FMR |= 1 << 0;

		// 设置CAN2SB=0：bit[13:8]清零后写入0，bank0~27全部划分给CAN2
		CAN1->FMR = (CAN1->FMR & ~(0x3F << 8)) | (0 << 8);

		// 去激活过滤器bank0（修改配置前必须先去激活）
		CAN1->FA1R &= ~(1 << 0);

		// 过滤器bank0位宽设为32位（FR1+FR2合成一个32位过滤器）
		CAN1->FS1R |= 1 << 0;

		// 过滤器bank0工作在屏蔽位模式（FR1=期望ID，FR2=MASK掩码）
		CAN1->FM1R &= ~(1 << 0);

		// 过滤器bank0关联到FIFO0（通过过滤的报文存入FIFO0）
		CAN1->FFA1R &= ~(1 << 0);

		// 期望匹配的ID设为0（配合MASK全0，实现接收所有报文）
		CAN1->sFilterRegister[0].FR1 = 0x00000000;

		// MASK掩码全0：所有ID位均不参与比较，任意报文均可通过
		CAN1->sFilterRegister[0].FR2 = 0x00000000;

		// 激活过滤器bank0，使配置正式生效
		CAN1->FA1R |= 1 << 0;

		// 过滤器组退出初始化模式（FINIT=0），进入正常工作模式
		CAN1->FMR &= ~(1 << 0);
