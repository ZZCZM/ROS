...
void KEY_Init(void)
{
    ...
    GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_DOWN;    // 内部下拉（默认低电平）
...
u8 click(void)
{
	...
	if(flag_key && (KEY == 1))  // 检测上升沿（从高到低）
    {
        flag_key = 0;  // 锁定检测标志
        return 1;      // 返回按下事件
    }
    else if(KEY == 0)  // 检测按键释放
    {
        flag_key = 1;  // 重置检测标志
    }
    return 0;  // 无有效动作
...
u8 click_N_Double(u8 time)
{
    ...
    // 实时更新长按计时
    if(KEY == 1)  Forever_count++;   // 按键持续按下时递增
...
u8 click_N_Double_MPU6050(u8 time)
{
    ...
    /* 实时更新持续按下计时器 */
    if(KEY == 1)  Forever_count++;  // 按键按下时递增
